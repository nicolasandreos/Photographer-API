...
export class AdministratorUserController {
  constructor(private readonly useCases: AdministratorUserUseCasesFactory) {}

  create = async (req: Request, res: Response) => {
    const request = createAdministratorUserRequestSchema.parse(req.body);
    const newAdministratorUserEntity =
      await this.useCases.createAdministratorUserUseCase.execute(request);
    const newAdministratorUserDTO =
      AdministratorUserMapperDTO.toCreateResponseDTO(
        newAdministratorUserEntity,
      );
  ...
  login = async (req: Request, res: Response) => {
    ...
    const loginAdministratorUserResponse =
      await this.useCases.loginAdministratorUserUseCase.execute(request);
